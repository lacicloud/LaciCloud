...
class BruteForceBlock {
	...
	private static $_db = [
		'driver' => "mysql",
		'host'	=> "localhost",
		'database'	=> "laci_corporations_users",
		'charset' => "utf8mb4",
		'auto_clear' => true
	];
	...
	private static function _databaseConnect(){
		...
		$db = new \PDO(self::$_db['driver'].
			...
			parse_ini_file("../secrets.ini")["db_user_bruteforceblock"], parse_ini_file("../secrets.ini")["db_password_bruteforceblock"]);
	...
	public static function addFailedLoginAttempt($user_id, $ip_address){
		...
		try{
			$stmt = $db->query('INSERT INTO user_failed_logins SET user_id = '.$user_id.', ip_address = "'.$ip_address.'", attempted_at = NOW()');
	...
	public static function getLoginStatus($options = null, $ip_address){
		//get db connection
		$db = BruteForceBlock::_databaseConnect();
		...
		try{
			$stmt = $db->query('SELECT MAX(attempted_at) AS attempted_at FROM user_failed_logins WHERE ip_address = "'.$ip_address.'"');
	...
	//clear the database
	public static function clearDatabase($ip_address){
		//get db connection
		$db = BruteForceBlock::_databaseConnect();
		
		//attempt to delete all records
		try{
			$stmt = $db->query('DELETE from user_failed_logins WHERE ip_address = "'.$ip_address.'" ');
			return true;
		} catch(\PDOException $ex){
			//return errors
			return $ex;
